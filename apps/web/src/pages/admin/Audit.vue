...
<template>
  ...
  <div class="card p-3 mb-4 admin-filter-bar">
    <select v-model="filter.action" class="admin-select w-full sm:w-auto">
    ...
    <input
      ...
      placeholder="操作者用户名"
      class="admin-input flex-1 min-w-32 sm:flex-none sm:w-40"
      @keydown.enter="load"
    />
    <input
      v-model="filter.target"
      placeholder="目标资源（如 order:Pxxx）"
      class="admin-input flex-1 min-w-40 sm:flex-none sm:w-60"
      @keydown.enter="load"
    />
    <input
      v-model="filter.since"
      type="datetime-local"
      class="admin-input flex-1 sm:flex-none"
    />
